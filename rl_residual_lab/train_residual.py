"""Train PPO on a fixed-interval residual pellet-injection controller."""
...
try:
    ...
    raise ImportError("stable-baselines3 is required to run train_residual.py") from exc

from RL.vec_env import BatchAsVecEnv
from rl_residual_lab.residual_env import make_residual_env_fn
...
class ResidualStatsLogger(BaseCallback):
    """Log actual step rewards and residual action summaries."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PPO residual policy on top of a fixed injection schedule.")
    parser.add_argument("--torax-config", default="config/ITER.py")
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--max-steps", type=int, default=1_000, help="Control horizon after the 2 s warmup.")
    parser.add_argument("--device", default=None)
    parser.add_argument("--seed", type=int, default=0)

    parser.add_argument("--warmup-steps", type=int, default=2_000, help="Warmup horizon before RL takes over.")
    ...
    parser.add_argument("--initial-residual-std", type=float, default=0.25)
...
def initialize_zero_residual_policy(model: PPO, initial_std: float) -> None:
    """Initialize the actor to output zero residuals with modest exploration."""
    ...
    if action_net is None or log_std is None or action_net.bias is None:
        print("[Warn] policy does not expose action_net/log_std, skip residual initialization.")
        return
    if action_net.bias.numel() != 2 or log_std.numel() != 2:
        print(
            f"[Warn] expected 2-D residual action policy, got bias={action_net.bias.numel()} "
            f"log_std={log_std.numel()}, skip residual initialization."
        )
        return
    ...
    print(f"[Init] Zero residual policy initialized with std={residual_std:.4f}")
...
def main() -> None:
    ...
    run_name = args.run_name or f"ppo_residual_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    initialize_zero_residual_policy(model, args.initial_residual_std)
    ...
    model.learn(
        ...
        callback=[checkpoint_cb, ResidualStatsLogger()],
    ...
    print(f"Training finished. Final residual model saved to: {final_path}.zip")
