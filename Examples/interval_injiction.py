"""Run parallel simulator in interval-injection mode (no SB3 wrapper)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Interval-injection debug run on raw simulator.")
    ...
    parser.add_argument("--inject-fraction", type=float, default=1.0)
...
def main() -> None:
    args = parse_args()
    run_name = args.run_name or f"interval_injiction_{time.strftime('%Y%m%d_%H%M%S')}"
    output_dir = resolve_project_path(args.log_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    if args.inject_every <= 0:
        raise ValueError("--inject-every must be > 0")
    if args.inject_duration <= 0:
        raise ValueError("--inject-duration must be > 0")
    if not (0.0 <= args.inject_fraction <= 1.0):
        raise ValueError("--inject-fraction must be in [0, 1]")
    ...
    active_window_history: list[float] = []
    ...
    for step in range(args.run_steps):
        ...
        in_window = (step % args.inject_every) < args.inject_duration
        if in_window:
            if args.inject_fraction >= 1.0:
                next_triggers = torch.ones((args.batch_size,), dtype=torch.bool, device=device)
            else:
                next_triggers = torch.rand(args.batch_size, device=device) < args.inject_fraction
        else:
            next_triggers = torch.zeros((args.batch_size,), dtype=torch.bool, device=device)
        ...
        active_window_history.append(1.0 if in_window else 0.0)

        if step % max(1, args.log_interval) == 0:
            elapsed = time.time() - start_time
            sps = (step + 1) / elapsed if elapsed > 0 else 0.0
            print(
                f"Step {step:5d} | window={'Y' if in_window else 'N'} | "
                f"inject(applied/cmd)={applied_inject_rate * 100:5.1f}%/{command_inject_rate * 100:5.1f}% | "
    ...
    elapsed = time.time() - start_time
    summary = {
        "mode": "interval_injiction",
        "run_name": run_name,
        "steps": args.run_steps,
        "batch_size": args.batch_size,
        "elapsed_sec": elapsed,
        "sps": args.run_steps / elapsed if elapsed > 0 else 0.0,
        "active_window_ratio": float(np.mean(active_window_history)),
