...
#
#
# class WeightedAvgBadInput(ValueError):
...
#         a = np.arctan2(z - self.z0, r - self.r0)
#         ind = a.argsort()
#         rad = rad[ind]
#         a.sort()
#
#         theta2 = np.append(a, a + 2.0 * np.pi)
...
#         if self.symmetric:
#             a = np.matrix(self.thetafine)
#         else:
#             a = np.matrix(self.thetafine)
#         cm = np.cos(nph.transpose() * a)
#         sm = np.sin(nph.transpose() * a)
...
#         raise ValueError('Windowed FFT FATAL ERROR! NO DATA SUPPLIED')
#
...
#         raise ValueError('Windowed FFT Error! len(t < nfft! Your data set is smaller than the requested FFT window.')
#     if overlap > (nfft - 1.0) / nfft:  # Check for input errors
#         raise ValueError('Windowed FFT Error! overlap too big! overlap>(1-nfft)/nfft')
...
#         raise ValueError('Windowed FFT Error! FFT bin-bin spacing too small (try reducing overlap)')
#
#     # Setup
...
#         warnings.warn('both cutoff_timescale and cutoff_omega have been specified!')
...
# #                 warn("WARNING: Constraints are not considered if only optimizing knot y values.")
...
#                     eps = 1e-3 * s0
#                 for xc, yc in xyc:
#                     cons.append({'type': 'eq', 'fun': lambda args, xc, yc: interpolate.splev(xc, form_tck0(args)) - yc})
#                 for xp, yp in xyp:
#                     cons.append({'type': 'eq', 'fun': lambda args, xp, yp: interpolate.splev(xp, form_tck0(args), der=1) - yp})
#                 # additional constraints for knot separation
#                 for i in range(1, nt):
#                     cons.append({'type': 'ineq', 'fun': lambda t, i: (t[i] - t[i - 1]) - s0})
...
#                 for xc, yc in xyc:
#                     cons.append(
#                         {'type': 'eq', 'fun': lambda args, xc, yc: interpolate.splev(xc, form_tck0(np.hstack((t0, args)))) - yc}
#                     )
#                 for xp, yp in xyp:
#                     cons.append(
#                         {
#                             'type': 'eq',
#                             'fun': lambda args, xp, yp: interpolate.splev(xp, form_tck0(np.hstack((t0, args))), der=1) - yp,
#                         }
#                     )
...
#                 raise RuntimeError("TerminateTrajectory")
#             dt_ds = 1.0 / ds_dt
...
# def _gaussian_kernel1d(sigma, order, r, causal=False):
#     """
...
#     """
#     sd = float(sigma)
#     # make the r of the filter equal to truncate standard deviations
